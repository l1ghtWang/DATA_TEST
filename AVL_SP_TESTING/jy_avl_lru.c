...
void printfInorder(Item_t *item)
{
	Item_t* temp = item;
	while(temp != NULL)
    {
    	printf("node: %d height: %d size: %d\n",temp->addrKey, temp->height, temp->size );
...
long long findItem(LRU_Cache_t* cache, long long ikey)
{
    ...
    if(s != NULL)
    {
    	...
    	if(d==10)
    	printfInorder(cache -> mostRecentItem);
...
void access(LRU_Cache_t* cache, Hist_t* hist, long long ikey)
{
	...
	if (rd == COLDMISS) {createItem(cache, ikey); }
		...
		printf("d is %d, ikey is %d, rd is %d\n",d,ikey, rd);
...
{	
	...
	assert(cache->mostRecentItem != item);
	Item_t *leftMost = cache->mostRecentItem;
	//item->parent = NULL;
	//if(cache->mostRecentItem != NULL)
	//printf("%d mostRecentItem: %d curr: %d\n",d, cache->mostRecentItem->addrKey, item->addrKey );
...
Item_t* avlRemove(LRU_Cache_t* cache, Item_t *item){
	...
	if(succ == NULL)
	{
		...
		//rebalance start at curr->parent
		rbItem = curr->parent;


	}
	else //succ is real node
	{
		if(curr->right == NULL) // curr dont have right subtree
		{
			...
			//rebalance start at succ
			rbItem = curr->parent;

		}
		else
		{
			if(succ->parent->left == succ) // succ is parent's left child
			{
			...
			{
				assert(succ == curr->right);
				//printf("hshs\n");
				curr->right = succ->right;
				if(succ->right != NULL)
					succ->right->parent = curr;

			}

			//rebalance start at succ's original parent
			rbItem = succ->parent;
